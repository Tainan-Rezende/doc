...
export default function GetTetherToExternalTester() {
    ...
    const handleConsultation = async (e) => {
        ...
        try {
            ...
            if (!Array.isArray(netData) || netData.length === 0) {
                throw new Error('Nenhuma rede blockchain disponível para esta conta.');
            }

            const selectedNetwork = netData[0];
            
            if (!selectedNetwork.cryptocurrencies || selectedNetwork.cryptocurrencies.length === 0) {
                 throw new Error(`A rede ${selectedNetwork.name} não possui criptomoedas configuradas.`);
            }

            const usdtObj = selectedNetwork.cryptocurrencies.find(
                item => item.cryptocurrency.symbol === 'USDT'
            );

            if (!usdtObj) {
                throw new Error(`A criptomoeda USDT não foi encontrada na rede ${selectedNetwork.name}.`);
            }

            const selectedCrypto = usdtObj.cryptocurrency;

            setEtapa('Calculando Taxas...');
    ...
    return (
        <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '15px', borderBottom: '1px solid var(--ifm-color-emphasis-200)', paddingBottom: '10px' }}>
                ...
                <h3 style={{ margin: 0, fontSize: '1.1rem' }}>Testar: Cotação USDT (External Wallet)</h3>
            </div>
            ...
            <form onSubmit={handleConsultation}>
                ...
                >
                    {loading ? etapa : 'Calcular Recebimento'}
                </button>
            </form>
