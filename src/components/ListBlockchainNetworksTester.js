...
export default function ListBlockchainNetworksTester() {
  ...
  const handleList = async (e) => {
    ...
    try {
      ...
      const response = await fetch('https://api.xgateglobal.com/deposit/company/blockchain-networks', {
        method: 'GET',
        headers: {
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '15px', borderBottom: '1px solid var(--ifm-color-emphasis-200)', paddingBottom: '10px' }}>
          ...
          <h3 style={{ margin: 0, fontSize: '1.1rem' }}>Testar: Listar Redes Blockchain</h3>
      </div>
      ...
      <form onSubmit={handleList} style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
        ...
        <button type="submit" disabled={loading} className="button button--primary button--block">
          {loading ? 'Carregando...' : 'Listar Redes Disponíveis'}
        </button>
      </form>
