    ...
    const handleCreateOrder = async (e) => {
        ...
        try {
            ...
            if (!currencyResponse.ok) {
                 throw new Error(`Erro ao buscar moedas: ${currencyList.message || currencyResponse.statusText}`);
            }

            if (!Array.isArray(currencyList) || currencyList.length === 0) {
                throw new Error('Lista de moedas retornou vazia.');
            }

            const selectedCurrency = currencyList[0];

            setEtapa('Criando Pedido...');
    ...
    return (
        <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '15px', borderBottom: '1px solid var(--ifm-color-emphasis-200)', paddingBottom: '10px' }}>
                ...
                <h3 style={{ margin: 0, fontSize: '1.1rem' }}>Testar: Criar Pedido de Depósito</h3>
            </div>
            ...
            <form onSubmit={handleCreateOrder}>
                ...
                >
                    {loading ? etapa : 'Gerar Pedido de Depósito'}
                </button>
            </form>
